...
?>
	...
	<table class="container" border="0">
			  ...
			  <table cellpadding="0" cellspacing="0" border="0">
							...
							&nbsp;
							Page Under:
							<select name="catId" id="pagecontentId" onchange="searchText()" class="SelectBox">
							...
							Language:<select name="langId" id="langId" onchange="searchText()" class="SelectBox">
							<option value="">All</option>
							...
							?>
								...
								Status:
						&nbsp;
							<select class="SelectBox" name="active" id="active_search"  onchange="javascript:window.location='<? $seoLink->getLink('pageContent',true); ?>?active='+this.value"><?php $os->onlyOption($statuslist,$active);	?>
							</select>
							&nbsp;
				 ...
				 <? if($wtAccess['addPage']){?>
				 <a href="javascript:void(0)"  class="addButton" onclick="os.editRecord('<? echo $DivIds['EDITPAGE']?>0') ">Add record</a>
				 <? } ?>
				 </span></div>
				 ...
				 <table  border="0" cellspacing="0" cellpadding="0" class="listTable" >
							...
							<tr class="border" id="selected<?php echo $c;?>" onclick="trSelected('<?php echo $c;?>','<?php echo  count($records);?>');"   onmouseover="javascript:miz.switchRow('<?php echo  $DivIds['BUTTONS']; ?>')">
								...
								Meta Description : <?  echo  $record['metaDescription'];?>
									...
									<td>
									Language: <b>  <? echo $os->getByFld('lang','langId',$record['langId'],'title');?></b> <br />
