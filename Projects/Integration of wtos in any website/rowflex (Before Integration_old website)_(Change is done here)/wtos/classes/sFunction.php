...
if(trim($sDate)!="")
		...
		$nextday  = mktime(0, 0, 0, $dm[1]  ,$dm[2]+1 , $dm[0]);
...
function showDate($date,$format='d-m-Y') // 2013-06-05 08:46:48 
{
if($date=='' || $date=='0000-00-00 00:00:00'|| $date=='0000-00-00')
{
    //  $dateResult=date($format,time());
	  $dateResult='';
}else{
...
function saveDate($date,$format='dmy',$newFormat='yyyy-mm-dd 00:00:00') // 2013-06-05 08:46:48 
{

   $dateResult=$this->dateC($date,$format,$newFormat);
	 ...
	 while($ts=mysql_fetch_row($t))
	  {
	    ...
	    $models=(!strpos($site['root'],$modelss))?'':$models;
