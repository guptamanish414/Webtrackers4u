...
class systemModel extends wtos 
{
	...
	function options($keyField,$valField,$tName,$where='',$orderby='',$limit='')
	{
	   ...
	   $list=array();
	        if($this->extraOptionField!='')
			{
			 $fldQ=" , ".$this->extraOptionField;
			}
	 
	 
	 
	  $l= $this->getT($tName," $keyField  , $valField  $fldQ ", $where,$orderby,$limit);
	  if(is_array($l))
	  {
	    ...
	    foreach($l as $val)
		{
		  ...
		  if($this->extraOptionField!='')
			{
			 $vals=$val[$valField]." -".$val[$this->extraOptionField];
			}
			else
			{
		      $vals=$val[$valField];
	...
	function editTextField($value,$table,$editFld,$idFld,$idVal , $inputNameID='changeStatusDD',$extraParams='class="tField" ')
	{
	    ...
	    $vars=	"'$table','$editFld','$idFld','$idVal'";		
		?>
		<input name="<? echo $inputNameID; ?>" id="<? echo $inputNameID; ?>" type="text" onchange="os.changeStatus(this,<? echo $vars?>)" value="<? echo $value ?>"  <? echo $extraParams; ?> > 
