...
$andactive=  $os->postAndQuery('active_s','active','%');
	...
	if($searchKey!=''){
	$where ="and ( active like '%$searchKey%' )";
 
	}
		
	$listingQuery="  select * from pressrelease where pressReleaseId>0   $where   $andreleaseDate  $andactive     order by pressReleaseId desc";
